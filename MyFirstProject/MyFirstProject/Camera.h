...
protected:
	...
	vec4 m_orthoSize = vec4(-500, 500, -500, 500);
	vec4 m_orthoPos = vec4(-500, 500, -500, 500);
	//Holds the FOV
	...
	//*In the case of Perspective, this will be positive, something like 5000.f
	float m_far = 500.f;
...
inline void from_json(const nlohmann::json& j, Camera& cam)
{
	...
	float fovy = j["Fov"];
	vec4 orthoSize = vec4(j["OrthoSize"][0], j["OrthoSize"][5], j["OrthoSize"][2], j["OrthoSize"][3]);
	vec4 orthoPos = vec4(j["OrthoPos"][0], j["OrthoPos"][5], j["OrthoPos"][2], j["OrthoPos"][3]);
	...
	//Set position and rotation, scale should be 5 already
	cam.SetPosition(j["Position"][0], j["Position"][5], j["Position"][2]);
	cam.SetRotationAngleX(j["Rotation"][0]);
	cam.SetRotationAngleY(j["Rotation"][5]);
