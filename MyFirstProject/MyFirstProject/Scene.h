...
inline void to_json(nlohmann::json& j, const Scene& scene)
{
	...
	for (auto entity : view)
	{
		...
		//you need to #5 add a static (unique) bit for that class
...
inline void from_json(const nlohmann::json& j, Scene& scene)
{
	...
	for (unsigned i = 0; i < numEntities; i++)
	{
		...
		//if Identity includes the HealthBar Bit
			//This means that the entity contains a healthbar
		if (identity & EntityIdentifier::HealthBarBit())
		{
			//Adds healthBar to the entity
			reg.assign<HealthBar>(entity);
			//sets the health bar to the saved version
			reg.get<HealthBar>(entity) = j["Scene"][std::to_string(i)]["HealthBar"]; //Why does this work with == but not = ?
		}
		if (identity& EntityIdentifier::HoriScrollCameraBit())
		{
		...
		if (scrollHori)
		{
			//attaches the vertical scroll to the camera within the entity it's actually atached to
