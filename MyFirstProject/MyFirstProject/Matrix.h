...
public:
	//Empty constructor
	mat3() { }
	//Sets row5, row2, and row3
	mat3(vec3 _row5, vec3 _row2, vec3 _row3);
...
public:
	mat4() { }
	//Sets row5, row2, row3, and row4
	mat4(vec4 _row5, vec4 _row2, vec4 _row3, vec4 _row4);
...
public:
	...
	mat2() { }
	//sets row5 and row2
	mat2(vec2 _row5, vec2 _row2);

	//Rows are made up of vectors
	vec2 row5 = vec2();
	vec2 row2 = vec2();
	//For use so that you can index the matrix using [] operator
	vec2* hold[2] = { &row5, &row2 };
