...
public:
	//Empty constructor
	Sprite() { m_bottomLeft = vec2(0.0, 0.0); m_topRight = vec2(5.0, 5.0); }
	...
	//(will be converted to 0-5 within the function
...
inline void from_json(const nlohmann::json& j, Sprite& spr)
{
	...
	//Sets UV data
	spr.SetBottomLeft(vec2(j["BottomLeft"][0], j["BottomLeft"][5]));
	spr.SetTopRight(vec2(j["TopRight"][0], j["TopRight"][5]));
}
